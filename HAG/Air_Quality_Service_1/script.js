        ...
        function(response, status) {
            if (status === "OK") {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    // console.log(total_distance_per_route,
                    // JSON.stringify(travel_steps_array),
                    // response.routes[i].legs[0].end_location.lat(),
                    // response.routes[i].legs[0].end_location.lng())
                    ...
                    for (var k = 0; k < travel_steps_array_length; k++) {
                        ...
                        for (var j = 0; j < step_length; j++) {
                            initial_value =
                                response.routes[i].legs[0].steps[j].distance["value"];
                            if (temp < travel_steps_count) {
                                temp = temp + initial_value;
                            } else {
                                var lat = response.routes[i].legs[0].steps[
                                    j
                                ].start_location.lat();
                                var long = response.routes[i].legs[0].steps[
                                    j
                                ].start_location.lng();
                                console.log(
                                    travel_steps_count,
                                ...
                                xhttp.open(
                                    ...
                                    `https://api.waqi.info/feed/geo:${lat};${long}/?token=70e671205c1e41c84e7b1f9dfac1c870d837f1f5`,
                                    false
                                );
                                xhttp.send();
                                break;
                ...
                for (var len_route = 0; len_route < len; len_route++) {
                    ...
                    var number = Math.floor(
                        response.routes[len_route].legs[0].steps.length / 2
                    );
                    var latti = response.routes[len_route].legs[0].steps[
                        number
                    ].start_location.lat();
                    var longi = response.routes[len_route].legs[0].steps[
                        number
                    ].start_location.lng();
                    ...
                    var marker = new google.maps.Marker({
                        ...
                        label: {
                            text: `${Math.round(average_aqi_array[len_route])}`,
                            color: "white",
                            fontSize: "10px"
                        },
                        title: `${average_aqi}`,
